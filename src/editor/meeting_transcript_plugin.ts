...
const meetingTranscriptPluginKey = new PluginKey<MeetingPluginState | null>("ulpasoMeetingTranscript");

function transactionTouchesMeetingRange(transaction: Transaction, from: number, to: number): boolean {
  let mappedFrom = from;
  let mappedTo = to;
  for (const stepMap of transaction.mapping.maps) {
    let touches = false;
    stepMap.forEach((oldStart, oldEnd) => {
      const deletesProtectedContent = oldEnd > oldStart && oldStart < mappedTo && oldEnd > mappedFrom;
      const insertsInsideProtectedContent = oldEnd === oldStart && oldStart > mappedFrom && oldStart < mappedTo;
  ...
  return new Plugin<MeetingPluginState | null>({
    ...
    state: {
      ...
      apply(transaction, previous) {
        const action = transaction.getMeta(meetingTranscriptPluginKey) as MeetingPluginAction | undefined;
        ...
        return {
          ...
          revealFrom: previous.revealFrom == null
            ? undefined
            : transaction.mapping.map(previous.revealFrom, -1),
          revealTo: previous.revealTo == null
            ? undefined
            : transaction.mapping.map(previous.revealTo, 1),
        };
      },
    },
    props: {
      decorations(state) {
        ...
        const revealTo = Math.max(revealFrom, Math.min(state.doc.content.size, meeting.revealTo ?? 0));
    ...
    view(view) {
      const syncPartialText = (currentView: EditorView) => {
        ...
        const element = meeting?.partial
          ? Array.from(currentView.dom.querySelectorAll<HTMLElement>(".meeting-transcript-partial"))
            .find((candidate) => candidate.dataset.meetingSession === meeting.sessionId) ?? null
          : null;
    ...
    filterTransaction(transaction, state) {
      const meeting = meetingTranscriptPluginKey.getState(state);
      if (!meeting || !transaction.docChanged || transaction.getMeta(meetingTranscriptPluginKey)) return true;
...
function setMeetingPluginState(transaction: Transaction, state: MeetingPluginState): Transaction {
  return transaction.setMeta(meetingTranscriptPluginKey, { type: "set", state } satisfies MeetingPluginAction);
}

function clearMeetingPluginState(transaction: Transaction, sessionId: string): Transaction {
  return transaction.setMeta(meetingTranscriptPluginKey, { type: "clear", sessionId } satisfies MeetingPluginAction);
}
