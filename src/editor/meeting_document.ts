...
function reconcileMeetingTranscriptSegments(
  ...
  const oldSpeakers: Array<number | null> = [];
  previous.forEach((segment, index) => {
    if (index) {
      oldUnits.push(" ");
      oldSpeakers.push(previous[index - 1].speaker ?? null);
    }
    for (const unit of Array.from(segment.text.trim())) {
      oldUnits.push(unit);
      oldSpeakers.push(segment.speaker ?? null);
    }
  });
  const newUnits = Array.from(nextText);
  if (!oldUnits.length) {
    return [{ speaker: currentSpeaker ?? null, text: nextText }];
  }
  ...
  const labels: Array<number | null> = new Array(newUnits.length).fill(null);
  ...
  return reconciled.length
    ? reconciled
    : [{ speaker: currentSpeaker ?? null, text: nextText }];
}
