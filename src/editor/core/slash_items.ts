...
const localizedSlashItemKeys: Record<string, [MessageKey, MessageKey]> = {
  "core-editor.heading-1": ["editor.slash.heading1.title", "editor.slash.heading1.description"],
  "core-editor.heading-2": ["editor.slash.heading2.title", "editor.slash.heading2.description"],
  "core-editor.heading-3": ["editor.slash.heading3.title", "editor.slash.heading3.description"],
  "core-editor.blockquote": ["editor.slash.blockquote.title", "editor.slash.blockquote.description"],
  "core-editor.code-block": ["editor.slash.code_block.title", "editor.slash.code_block.description"],
  "core-editor.horizontal-rule": ["editor.slash.horizontal_rule.title", "editor.slash.horizontal_rule.description"],
  "core-editor.image": ["editor.slash.image.title", "editor.slash.image.description"],
  "core-editor.table": ["editor.slash.table.title", "editor.slash.table.description"],
  "core-editor.bullet-list": ["editor.slash.bullet_list.title", "editor.slash.bullet_list.description"],
  "core-editor.ordered-list": ["editor.slash.ordered_list.title", "editor.slash.ordered_list.description"],
  "core-editor.checkbox-list": ["editor.slash.checkbox_list.title", "editor.slash.checkbox_list.description"],
};
...
function getEditorSlashItems(): EditorSlashItem[] {
  // HMR can re-run a plugin's registration while the registry module stays
  // alive. Keep the latest entry for each stable command id so the menu never
  // renders duplicate commands. The older entry remains in the array and is
  // revealed again if the newer registration is disposed.
  const latestById = new Map<string, RegisteredEditorSlashItem>();
  for (const entry of registeredSlashItems) {
    latestById.set(entry.item.id, entry);
  }
  ...
  return [...latestById.values()]
    ...
    .map((entry) => {
      const keys = localizedSlashItemKeys[entry.item.id];
      return keys ? { ...entry.item, title: t(keys[0]), description: t(keys[1]) } : entry.item;
    });
}
