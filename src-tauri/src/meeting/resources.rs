...
pub(crate) fn inspect(resource_dir: &Path, app_data: &Path) -> MeetingResourceStatus {
    let runtime_ready = runtime_candidates(resource_dir, app_data)
        .iter()
        .any(|path| path.join("bin/python3").exists());
...
fn runtime_candidates(resource_dir: &Path, app_data: &Path) -> [PathBuf; 3] {
    [
        resource_dir.join("asr-runtime"),
        resource_dir.join("resources/asr-runtime"),
...
mod tests {
    ...
    fn test_root() -> PathBuf {
        std::env::temp_dir().join(format!("ulpaso-resource-status-{}", Uuid::new_v4()))
    }
    ...
    fn recognizes_bundled_runtime_and_both_complete_models() {
        ...
        std::fs::create_dir_all(resources.join("asr-runtime/bin")).expect("create runtime");
        std::fs::write(resources.join("asr-runtime/bin/python3"), b"python")
