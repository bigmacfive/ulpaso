...
mod tests {
    use super::*;
    use crate::meeting::audio::SAMPLE_RATE;
    use uuid::Uuid;

    #[test]
    fn repairs_pcm_wav_left_unfinalized_by_an_interrupted_exit() {
        let path = std::env::temp_dir().join(format!("ulpaso-wav-repair-{}.wav", Uuid::new_v4()));
    ...
    fn leaves_finalized_or_unrelated_files_untouched() {
        let path = std::env::temp_dir().join(format!("ulpaso-wav-valid-{}.wav", Uuid::new_v4()));
