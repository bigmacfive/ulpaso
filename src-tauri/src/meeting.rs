...
use uuid::Uuid;
use worker_protocol::{
    append_worker_pcm, f32_bytes, worker_feeder_loop, write_worker_frame, WorkerInput,
    WorkerQueueMessage,
};
...
impl MeetingController {
    ...
    pub fn start(
        ...
        thread::spawn(move || {
            if let Err(error) = controller.prepare_worker() {
                controller.fail("worker_prepare", error);
            }
        });
        Ok(self.status())
    }

    pub fn prepare(&self) -> Result<MeetingStateSnapshot, String> {
        if self.status().phase == "idle" {
            self.set_state(
                "preparing",
                None,
                Some("Checking the transcription engine"),
                None,
            );
            let controller = self.clone();
            thread::spawn(move || match controller.ensure_runtime() {
                Ok(_) => controller.set_idle("The transcription engine is ready"),
                Err(error) => controller.fail("runtime_prepare", error),
            });
        }
        Ok(self.status())
    }
    ...
    fn prepare_worker(&self) -> Result<(), String> {
        let python = self.ensure_runtime()?;
        let resource_dir = self
            .app
            .path()
            .resource_dir()
        ...
        if std::env::var("ULPASO_ASR_MOCK").ok().as_deref() == Some("1") {
            command.arg("--mock");
        }
        ...
        let controller = self.clone();
        thread::spawn(move || controller.read_worker(stdout));
        Ok(())
    }

    fn ensure_runtime(&self) -> Result<PathBuf, String> {
        if let Ok(path) = std::env::var("ULPASO_ASR_PYTHON") {
        ...
        let resource_dir = self
            ...
            .map_err(|error| format!("Could not locate the app resource folder: {error}"))?;
        for candidate in [
            resource_dir.join("asr-runtime/bin/python3"),
            resource_dir.join("resources/asr-runtime/bin/python3"),
        ] {
            if candidate.exists() {
        ...
        if python.exists() && runtime_dir.join(".ulpaso-ready").exists() {
            return Ok(python);
        }
        ...
        let status = Command::new("/bin/zsh")
            ...
            .arg(&runtime_dir)
            .status()
            .map_err(|error| format!("Could not start MLX runtime installation: {error}"))?;
    ...
    fn read_worker(&self, stdout: impl std::io::Read) {
        ...
        for line in reader.lines() {
            ...
            self.handle_worker_event(value);
        }
        ...
        if !matches!(phase.as_str(), "idle" | "error") {
            self.fail(
                "worker_exit",
                "The local transcription engine closed unexpectedly. Audio was kept in the recovery folder"
                    .into(),
            );
        }
    }

    fn handle_worker_event(&self, value: Value) {
        match value.get("type").and_then(Value::as_str).unwrap_or("") {
            ...
            "transcript" => {
                let session_id = self.status().session_id.unwrap_or_default();
                ...
                if diagnostics_enabled()
                    && (!payload.stable_text.is_empty() || !payload.unstable_text.is_empty())
                {
                    eprintln!(
                        "[meeting-transcript] stable={:?} unstable={:?}",
                        payload.stable_text, payload.unstable_text
                    );
                }
                let _ = self.app.emit("meeting://transcript", payload);
            }
            "finalizing" => {
                let progress = value
                    .get("progress")
            ...
            "final" => {
                ...
                if diagnostics_enabled() {
                    eprintln!(
                        "[meeting-final] text={:?} segments={:?}",
                        payload.stable_text, payload.segments
                    );
                }
                let _ = self.app.emit("meeting://transcript", payload);
                #[cfg(debug_assertions)]
                let keep_audio =
                    std::env::var("ULPASO_KEEP_MEETING_AUDIO").ok().as_deref() == Some("1");
    ...
    fn restart_worker_after_exit(&self) -> bool {
        ...
        thread::spawn(move || {
            if let Err(error) = controller.prepare_worker() {
                controller.fail("worker_recovery", error);
            }
        });
        true
    }
...
fn spawn_mock_capture(sender: Sender<CaptureEvent>, system_only: bool) {
    thread::spawn(move || {
        ...
        let stall_after_chunks = std::env::var("ULPASO_AUDIO_MOCK_STALL_AFTER_CHUNKS")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(0);
        let mut chunks_sent = 0usize;
        let max_chunks = std::env::var("ULPASO_AUDIO_MOCK_MAX_CHUNKS")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(0);
        let interval_ms = std::env::var("ULPASO_AUDIO_MOCK_INTERVAL_MS")
            .ok()
...
fn find_asr_resource(resource_dir: &Path, name: &str) -> Result<PathBuf, String> {
    ...
    for candidate in [
        resource_dir.join("asr").join(name),
        resource_dir.join("resources/asr").join(name),
        manifest.join("resources/asr").join(name),
    ] {
        if candidate.exists() {
            return Ok(candidate);
        }
...
#[tauri::command]
pub fn meeting_status(controller: tauri::State<'_, MeetingController>) -> MeetingStateSnapshot {
    controller.status()
}

#[tauri::command]
pub fn meeting_resources(
...
#[tauri::command]
pub fn meeting_open_settings(error_code: Option<String>) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
