...
// A user has to bring Ulpaso to the foreground to start notes manually, so a
// synchronous environment read would see Ulpaso rather than the meeting app.
...
impl MeetingDetectionController {
    ...
    /// observed recently enough to survive the focus handoff into Ulpaso. The
    ...
    pub fn spawn(self) {
        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                self.remember_candidate(candidate.as_ref());
...
fn read_environment() -> MeetingEnvironment {
    ...
    match std::env::var("ULPASO_TEST_MEETING_ENVIRONMENT")
        .ok()
        .as_deref()
    {
        Some("google-meet") => {
            return MeetingEnvironment {
                ...
                window_title: "Meet - ulpaso-e2e".into(),
                window_id: Some(42),
            };
        }
        Some("zoom") => {
...
#[tauri::command]
pub fn meeting_detection_capture_target(
    controller: tauri::State<'_, MeetingDetectionController>,
) -> Option<MeetingCaptureTarget> {
